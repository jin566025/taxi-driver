...
gulp.task('compress-js',function (callback) {       //- 创建一个名为compress-js的task
    gulp.src(['fleet-footed-driver/js/**/*.js'])             //- 需要处理的js文件，放到一个字符串数组里
        .pipe(uglify())                             //- 压缩文件
        .pipe(rev())                                //- 文件名加MD5后缀
...
gulp.task('compress-css', function(callback) {      //- 创建一个名为compress-css的task
    gulp.src(['fleet-footed-driver/css/**/*.css'])           //- 需要处理的css文件，放到一个字符串数组里
...
gulp.task('rev-html',['compress-css','compress-js'], function() {          //- compress-css和compress-js任务执行完毕再执行rev-index任务
    ...
    gulp.src(['rev-css/*.json','rev-js/*.json', 'fleet-footed-driver/**/*.html'])   //- 读取两个rev-manifest.json文件以及需要进行css和js名替换的html文件
    ...
    gulp.src(['rev-css/*.json','rev-js/*.json', 'fleet-footed-driver/pages/**/*.html'])     //- 读取两个rev-manifest.json文件以及需要进行css和js名替换的html文件
        .pipe(revCollector())                                                      //- 执行文件内css和js名的替换
        .pipe(gulp.dest('dist/pages'));                                            //- 替换后的html文件输出的目录
});

/*压缩并复制图片*/
gulp.task('compress-img',function () {
    gulp.src('fleet-footed-driver/img/**/*.*')    //原图片的位置
